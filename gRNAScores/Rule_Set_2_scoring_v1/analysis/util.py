...
def ndcgk(relevances, rank=20):
    ...
    if best_dcg == 0:
        return 0.
    return old_div(dcg(relevances, rank), best_dcg)
...
def plot_all_metrics(metrics, gene_names, all_learn_options, save, plots=None, bottom=0.19):
    ...
    for i, method in enumerate(metrics.keys()):
        ...
        for metric in list(metrics[method].keys()):
            ...
            if 'global' in metric:
                plt.figure(metric)
                plt.bar([i], metrics[method][metric], 0.9, color=plt.cm.Paired(old_div(1.*i,len(list(metrics.keys())))), label=method)
            else:
                if plots == None or 'gene level' in plots:
                    plt.figure(metric)
                    plt.bar(ind+(i*width), metrics[method][metric], width, color=plt.cm.Paired(old_div(1.*i,len(list(metrics.keys())))), label=method)
