...
"""
def targetGene(geneName, geneGB, codonOptimize="T. gondii", HRannotated=False, lengthLHR=[450,500,750], lengthRHR=[450,500,750], gibsonHomRange=[20,30,40], optimRangeLHR=[-20,20], optimRangeRHR=[-20,20], endSizeLHR=40, endSizeRHR=40, endTempLHR=55, endTempRHR=59, gibTemp=65, gibTDif=5, maxDistLHR=500, maxDistRHR=500, minGBlockSize=250, maxGBlockSize=1000, filterCutSites=[cut_FseI,cut_AsiSI,cut_IPpoI,cut_ISceI,cut_AflII,cut_AhdI,cut_BsiWI,cut_NheI], useFileStrs=False, codonSampling=False, minGRNAGCContent=0.3, onTargetMethod="azimuth", minOnTargetScore=30, offTargetMethod="cfd", minOffTargetScore=0.5, maxOffTargetHitScore=35, enzyme="Cas9", PAM="NGG", gBlockDefault=True, plasmidType="pSN054", haTag=True, sigPep=False, setCoding="Auto", bulkFile=False, prefix="", outputDir='', basePlasmid=None, basePlasmidFile=None, basePlasmidName=None, locationType='3prime'): # cfd, 0.5, 35; hsu, 75, 5
    ...
    if '' in filterCutSites:
        filterCutSites.remove('')
    ...
    if len(geneList) > 0: # if genes found,
        ...
        if plasmidType == "pSN054" or plasmidType == "pSN054_V5": # if using 3' plasmid
            ...
            plasmid = basePlasmid;
            target3Prime = locationType=='3prime'; # target correct end
        ...
        if HRannotated: # if using manual annotations
            ...
            if not ( (plasmidType=="pSN150-KO" or (plasmidType=="custom" and locationType=="center")) and not len(gRNA['out'].label)>0 ): # if not,
                ...
                gRNA = chooseGRNA(geneGB, gene, searchRange=[-maxDist1,maxDist2], PAM=PAM, minGCContent=minGRNAGCContent, minOnTargetScore=minOnTargetScore, onTargetMethod=onTargetMethod, minOffTargetScore=minOffTargetScore, offTargetMethod=offTargetMethod, maxOffTargetHitScore=maxOffTargetHitScore, gBlockOverlapSize=gibsonHomRange[1], codingGene=codingGene, enzyme=enzyme, closestGene=closestGene, target3Prime=target3Prime, targetCenter=targetCenter, targetRegionOverride=(plasmidType=="custom"), filterCutSites=filterCutSites); # chooses gRNA.
        ...
        if (plasmidType=="pSN150-KO" or (plasmidType=="custom" and locationType=="center")) and not len(gRNA['out'].label)>0: # if knocking out and no gRNA found,
            gRNA = chooseGRNA(geneGB, gene, searchRange=[gene.index[0]-gene.index[1],0], PAM=PAM, minGCContent=minGRNAGCContent, minOnTargetScore=minOnTargetScore, onTargetMethod=onTargetMethod, minOffTargetScore=minOffTargetScore, offTargetMethod=offTargetMethod, maxOffTargetHitScore=maxOffTargetHitScore, gBlockOverlapSize=gibsonHomRange[1], codingGene=codingGene, enzyme=enzyme, closestGene=closestGene, target3Prime=True, targetCenter=False, targetRegionOverride=(plasmidType=="custom"), filterCutSites=filterCutSites); # search through whole gene from start as well
        ...
        if len(gRNA.label) > 0: # if gRNA found,
            ...
            if LHR["out"] is None or RHR["out"] is None and not HRannotated: # if searches fail and HR's not provided,
                ...
                if plasmidType != 'pSN150-KO' and not (plasmidType=="custom" and locationType=="center"): # if not knocking out,
                    ...
                    recoded = chooseRecodeRegion(geneGB, gene, offTargetMethod, pamType=PAM, orgCodonTable=codonUsageTables[codonOptimize],codonSampling=codonSampling, gRNATableString=outputDic["gRNATable"], target3Prime=target3Prime, haTag=False, targetRegionOverride=(plasmidType=='custom'), flankingSeqsRec=flankingSeqsRec, filterCutSites=filterCutSites); # defines region to be recoded, returns recoded sequence
                ...
                if haTag and not plasmidType == "pSN150-KO" and not plasmidType=="custom": # if using HA tags,
                    ...
                    outputDicHA = postProcessPlasmid(geneName, geneGB, gene, plasmidArmedHA, recodedHA, outputDicHA, path, useFileStrs, geneOrientationNegative=geneOrientationNegative, plasmidType=plasmidType, enzyme=enzyme, gibsonHomRange=gibsonHomRange, gibTemp=gibTemp, gibTDif=gibTDif, minGBlockSize=minGBlockSize, haTag=True, gBlockDefault=gBlockDefault, prefix=prefix, gRNATableString=outputDic["gRNATable"]); # generate and annotate assembly info
        ...
        elif not useFileStrs: # if no gRNAs found,
            output(outputDic["logFileStr"], path + "/" + prefix + "Message_File_" + geneName +"_"+ plasmidType + "_" + enzyme + ".txt",wipe=True); # saves message log to file
...
def postProcessPlasmid(geneName, geneGB, gene, plasmidArmed, recoded, outputDic, path, useFileStrs, geneOrientationNegative, plasmidType="pSN054", enzyme="Cas9", gibsonHomRange=[30,40,50], gibTemp=65, gibTDif=5, minGBlockSize=270, haTag=False, gBlockDefault=True, prefix="",gRNATableString=None):
    if len(plasmidArmed.findAnnsLabel(gene.label + " gRNA")) > 0: # if there actually is a gRNA,
    ...
    elif len(plasmidArmed.findAnnsLabel(gene.label + " Recoded")) > 0 and len(recoded.seq) >= gibsonHomRange[0]/2: # if length of recoded region greater or equal to half of minimum size of homology region,
    ...
    if len(plasmidArmed.findAnnsLabel(gene.label + " gRNA")) > 0: # if there actually is a gRNA,
        ...
        if plasmidType == "pSN150": # if using pSN150 instead of pSN054,
            ...
            if len(plasmidArmed.findAnnsLabel("sgRNA Cassette")) > 0:
